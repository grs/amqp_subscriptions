...
from proton.reactor import Container, DurableSubscription, LinkOption

class GlobalSubscription(LinkOption):
    def apply(self, link):
        link.source.capabilities.put_symbol('global')

class SharedSubscription(LinkOption):
    def apply(self, link):
        link.source.capabilities.put_symbol('shared')
...
parser.add_option("-i", "--id", default="myclient",
                  help="client's connection identifier (default %default)")
parser.add_option("-s", "--subscription", default="mysubscription",
